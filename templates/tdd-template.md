...
> 当前方案负责人完成本方案后，Standard 先由 Liu 技术审核，Rigorous 先由 Architect 设计或独立审核，再使用 `tdd-proposal` 完成结构与一致性校验。随后向用户展示职责目录树、设计覆盖版本、风险和方案摘要；用户确认职责、范围、取舍和残余风险，不替代技术正确性审核。只有用户明确确认后，才允许进入开发前设计补水。
