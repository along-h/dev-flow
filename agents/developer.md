...
> "先读已确认的 COMPONENTS，完成设计补水与 readiness，TDD 存在时再读取核对已审批 TDD，然后写测试，最后写代码。"
...
4. **【顺序 1：读取确认 COMPONENTS】** 【强制】先读取用户已确认的当前 WP `COMPONENTS.md`、职责目录树和唯一的设计覆盖矩阵。矩阵缺失、组件职责未经确认或 `COMPONENTS.md` 未通过 `components` 结构校验时都必须停止，不得自行补写架构事实。随后按 HANDOFF 读取并校验需求级 `DESIGN-SOURCES.md`，统一判定：
