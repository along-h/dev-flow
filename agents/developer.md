...
1. **当前任务目标**（一句话）
2. **技术决策摘要**（组件树、数据流、API 契约；`fast` 路径使用会话内精简版本）
3. **硬性约束**（技术栈、目录结构、命名约定）
4. **TDD 定向读取路径**：当前 WP 的 `TDD.md`（存在时按 HANDOFF 读取）
...
4. 按 HANDOFF 读取并校验需求级 `DESIGN-SOURCES.md`；文件缺失或状态不合法时停止开发并返回需求分析阶段重建
5. 根据状态执行唯一对应分支：
   - `inactive`：不得向用户索要设计稿；读取项目现有组件、样式 Token 和同类页面作为视觉基线
   - `waived`：确认登记表包含用户明确原话及当前任务适用范围；不得继续索要设计稿，交付时标记人工视觉验收
   - `required`：仅检查当前工作包涉及的模块，执行下面的即时补水门禁
6. `required` 状态下，对每个当前工作包模块：
   - 先从已提供的顶层设计源和设计工具中自行定位精确节点，不得把可查事实转问用户
   - 开发前重新读取精确模块设计源，刷新需求级 `design/{模块名}.md`，避免旧提取结果或对话截断造成信息丢失
   - 使用校验器验证刷新后的模块设计规格
   - 将精确链接、提取时间、完整度和规格文件路径写回需求级 `DESIGN-SOURCES.md`
7. 如果仍无法获得精确布局、尺寸、间距、文字处理或适用组件状态：
   - 一次性列出当前工作包所有缺失模块和缺失信息，向用户请求对应小模块的具体设计稿链接
   - 将状态写为 `incomplete` / `blocked`，停止当前工作包；不得生成组件代码或用猜测补齐设计
   - 用户补充后重新执行第 6 步；用户明确表示不需要设计稿时切换为 `waived` 并记录原话
8. 只有当前工作包所有模块规格均为 `complete`，或整体状态为 `inactive` / `waived`，才能进入实现
9. 读取当前治理路径实际存在的 PRD 或精简需求基线，以及本工作包模块规格
10. 当前 WP 的 `COMPONENTS.md` 存在时按 HANDOFF 读取，确认哪些组件标记为“复用（已有）”；`fast` 路径直接依据组件切片和精简技术决策，不虚构缺失文件
11. 读取 PRD 的事实/假设表、TDD 的风险评估和架构对抗审查结论，提取系统不变量与待验证风险
...
`required` 状态下，每个模块实现完成后必须对照最新模块设计规格逐项自检；`inactive` 状态对照项目现有视觉基线；`waived` 状态记录人工视觉验收结果：
