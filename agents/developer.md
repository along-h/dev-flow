...
### 第一步：类型定义先行（引用全局类型，不重复定义）

> **多 UC 工作量时**：读取 `artifacts/GLOBAL-ARCHITECTURE.md`，使用全局统一数据模型，不要在该 UC 内重新定义已有类型。

在写任何组件之前，先确认类型定义：
- 多 UC 工作量：读取全局 `types/` 下的统一数据模型，只定义该 UC 特有的类型
- 单 UC 工作量：创建该模块的类型定义文件
...
- 部分失败：跨接口或跨 UC 操作部分成功时有一致性与恢复策略。
